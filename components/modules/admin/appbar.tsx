...
export function Appbar() {
  ...
  return (
    <header className={`flex justify-between w-full items-center gap-x-10 px-10 mt-7 mb-4 ${
      isDashboardPage ? 'h-38 md:h-20' : 'h-24 md:h-10'
    }`}>
      {isDashboardPage && (
        <div>
      ...
      <div className={`flex items-center gap-4 ${!isDashboardPage ? "w-full justify-between" : ""}`}>
        <div className="relative">
          ...
          <Input 
            ...
            className={`w-full min-w-[200px] rounded-full pl-10 bg-white ${
              !isDashboardPage ? 'md:w-[500px]' : 'md:w-[300px]'
            }`}
          />
        </div>
